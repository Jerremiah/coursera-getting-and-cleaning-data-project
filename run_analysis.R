...
mergedata[,2] <- factor(act[mergedata[,2],2])
names(mergedata)[2:ncol(mergedata)] <- c("activities", as.character(features$signals))


# Answer5
library(reshape2)
melted_mergedata <- melt(mergedata, id = c("subject", "activities"))
mean_melted_mergedata <- dcast(allData.melted, subject + activities ~ variable, mean)

View(mean_melted_mergedata)

write.table(mean_melted_mergedata, "mean_melted_mergedata.txt", row.name=FALSE)
